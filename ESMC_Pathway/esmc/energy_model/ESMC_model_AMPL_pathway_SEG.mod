...
#	Pathway extension and intra-day segmentation: Pablo Jimenez Zabalaga
#
...
param n_seg > 0 default 24;             # number of intra-day segments (24 = hourly)
set PERIODS := 1 .. (365 * n_seg);      # segmented periods (one per segment per day); n_seg=24 -> 8760
set HOURS := 1 .. n_seg;                # intra-day SEGMENTS (variable duration t_op), not clock hours
...
set SET_INIT_SOL := {"F_up_to", "F_new_up_to", "F_decom_up_to", "F_old_up_to", "F_used_year_start_next", "R_year_local_up_to", "R_year_exterior_up_to", "F_start_next"};
...
param state_of_charge_ev {V2G, HOURS, TYPICAL_DAYS} >= 0 default 0;  # 3D: minimum SOC per (segment, typical day)
...
param t_h_td_count {h in HOURS, td in TYPICAL_DAYS} := card {t in PERIODS: (t,h,td) in T_H_TD};	# yearly count of each (h,td)
...
param F_START_TOL := 1e-6;  # relative tolerance to avoid numerical errors
...
var R_year_local {YEARS, REGIONS, RESOURCES} >= 0, default 0;
var R_year_exterior {YEARS, REGIONS, RESOURCES} >= 0, default 0;
...
var EUT_total {YEARS_WND diff YEAR_ONE, REGIONS, END_USES_TYPES};	# total demand per end-use type (see def_EUT_total)
...
subject to op_cost_calc {y in YEARS_UP_TO union YEARS_WND, c in REGIONS, i in RESOURCES}:
	C_op [y,c,i] = c_op_local [y,c, i] * R_year_local [y,c, i] + c_op_exterior [y, i] * R_year_exterior [y,c, i];
...
subject to gwp_op_calc {y in YEARS_UP_TO union YEARS_WND, c in REGIONS, i in RESOURCES}:
	GWP_op [y,c,i] = gwp_op_local [y,c, i] * R_year_local [y,c, i] + gwp_op_exterior [y, i] * R_year_exterior [y,c, i];

subject to co2_net_calc {y in YEARS_UP_TO union YEARS_WND, c in REGIONS, i in RESOURCES}:
	CO2_net [y,c,i] = co2_net [y, i] * (R_year_local [y,c, i] + R_year_exterior [y,c, i]);
...
subject to storage_level {y in YEARS_WND diff YEAR_ONE, c in REGIONS, j in STORAGE_TECH, t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]}:
	Storage_level [y,c, j, t] = (if t == 1 then
		Storage_level [y,c, j, card(PERIODS)] * (1.0 -  storage_losses[y, j])^(t_op [h, td])
		...
		Storage_level [y,c, j, t-1] * (1.0 -  storage_losses[y, j])^(t_op [h, td])
		+ t_op [h, td] * (   (sum {l in LAYERS: storage_eff_in [y, j,l] > 0}  (Storage_in [y,c, j, l, h, td]  * storage_eff_in  [y, j, l]))
		                   - (sum {l in LAYERS: storage_eff_out [y, j,l] > 0} (Storage_out [y,c, j, l, h, td] / storage_eff_out [y, j, l])))
		);
...
subject to freight_of_exchanges_grouped {y in YEARS_WND diff YEAR_ONE, c1 in GROUPED_REGIONS}:
    Exch_freight[y,c1] =
      ...
      + sum {c2 in REGIONS diff GROUPED_REGIONS} (Exch_freight_border[y,c1,c2] + Exch_freight_border[y,c2,c1]) / 2
      + sum {c2 in REGIONS diff GROUPED_REGIONS, c3 in REGIONS diff GROUPED_REGIONS:
             (c1,c2) in RELATED_PAIRS} Exch_freight_border[y,c2,c3] / 2;
...
subject to EV_storage_min_SOC {y in YEARS_WND diff YEAR_ONE, c in REGIONS, j in V2G, i in EVs_BATT_OF_V2G[j], t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]}:
	Storage_level [y,c, i, t] >= F [y,c,i] * state_of_charge_ev[j,h,td];
...
subject to Minimum_GWP_reduction {y in YEARS_WND diff YEAR_ONE, c in REGIONS}:
	...
	sum{r in RESOURCES} (CO2_net [y,c,r]) - sum {j in TECHNOLOGIES diff STORAGE_TECH: layers_in_out[y,j,"CO2_CAPTURED"] > 0} (-layers_in_out[y,j,"CO2_ATM"] * sum {t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} (F_t[y,c,j,h,td] * t_op[h,td])) <= gwp_limit[y,c];
...
subject to Minimum_GWP_reduction_global {y in YEARS_WND diff YEAR_ONE}:
	...
	sum{c in REGIONS, r in RESOURCES} (CO2_net [y,c,r]) - sum {c in REGIONS, j in TECHNOLOGIES diff STORAGE_TECH: layers_in_out[y,j,"CO2_CAPTURED"] > 0} (-layers_in_out[y,j,"CO2_ATM"] * sum {t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} (F_t[y,c,j,h,td] * t_op[h,td])) <= gwp_limit_overall[y];
...
# --- def_EUT_total: total demand of each end-use type, computed once per (y,c,eut) ---
# t_op is applied to F_t, not to resources (as in the original model).
subject to def_EUT_total {y in YEARS_WND diff YEAR_ONE, c in REGIONS, eut in END_USES_TYPES}:
	EUT_total [y,c,eut] = (sum {j2 in TECHNOLOGIES_OF_END_USES_TYPE[eut], h in HOURS, td in TYPICAL_DAYS} (F_t [y,c, j2, h, td] * t_op[h,td] * t_h_td_count[h,td])
	+ sum {r in RESOURCES, h in HOURS, td in TYPICAL_DAYS} (layers_in_out [y, r, eut] * (R_t_local [y,c, r, h, td] + R_t_exterior [y,c, r, h, td] + R_t_import [y,c, r, h, td] - R_t_export [y,c, r, h, td]) * t_h_td_count[h,td]));
...
subject to f_max_perc {y in YEARS_WND diff YEAR_ONE, c in REGIONS, eut in END_USES_TYPES, j in TECHNOLOGIES_OF_END_USES_TYPE[eut]}:
	sum {h in HOURS, td in TYPICAL_DAYS} (F_t [y,c,j,h,td] * t_op[h,td] * t_h_td_count[h,td])
	<= fmax_perc [y,c,j] * EUT_total [y,c,eut];
...
subject to f_min_perc {y in YEARS_WND diff YEAR_ONE, c in REGIONS, eut in END_USES_TYPES, j in TECHNOLOGIES_OF_END_USES_TYPE[eut]}:
	sum {h in HOURS, td in TYPICAL_DAYS} (F_t [y,c,j,h,td] * t_op[h,td] * t_h_td_count[h,td])
	>= fmin_perc [y,c,j] * EUT_total [y,c,eut];
...
# F_up_to: Installed capacity for all years optimized so far
var F_up_to {YEARS_UP_TO, REGIONS, TECHNOLOGIES} >= 0;
subject to store_F_up_to {y in YEARS_UP_TO, c in REGIONS, j in TECHNOLOGIES}:
    F_up_to[y,c,j] = F[y,c,j];

# Stores F[YEAR_ONE] to fix it between windows
var F_start {YEARS, REGIONS, TECHNOLOGIES} >= 0, default 0;
# Captures F[YEAR_ONE_NEXT] for the next window
var F_start_next {YEAR_ONE_NEXT, REGIONS, TECHNOLOGIES} >= 0;
subject to store_F_start_next {y in YEAR_ONE_NEXT, c in REGIONS, j in TECHNOLOGIES}:
    F_start_next[y,c,j] = F[y,c,j];

# Keeps F[YEAR_ONE] close to the fixed value (with tolerance)
subject to fix_F_start_lower {y in YEAR_ONE diff {"YEAR_2015"}, c in REGIONS, j in TECHNOLOGIES}:
    F[y,c,j] >= F_start[y,c,j] - F_START_TOL * (1 + F_start[y,c,j]);

subject to fix_F_start_upper {y in YEAR_ONE diff {"YEAR_2015"}, c in REGIONS, j in TECHNOLOGIES}:
    F[y,c,j] <= F_start[y,c,j] + F_START_TOL * (1 + F_start[y,c,j]);

# F_new_up_to: New capacity for all phases optimized so far (including 2015_2021 initial)
var F_new_up_to {PHASE_UP_TO union {"2015_2021"}, REGIONS, TECHNOLOGIES} >= 0;
subject to store_F_new_up_to {p in PHASE_UP_TO union {"2015_2021"}, c in REGIONS, j in TECHNOLOGIES}:
    F_new_up_to[p,c,j] = F_new[p,c,j];

# F_old_up_to: Retired capacity for all phases optimized so far
var F_old_up_to {PHASE_UP_TO, REGIONS, TECHNOLOGIES} >= 0, default 0;
subject to store_F_old_up_to {p in PHASE_UP_TO, c in REGIONS, j in TECHNOLOGIES}:
    F_old_up_to[p,c,j] = F_old[p,c,j];

# F_decom_up_to: Decommissioned capacity for all phases optimized so far
var F_decom_up_to {PHASE_UP_TO, PHASE_UP_TO union {"2015_2021"}, REGIONS, TECHNOLOGIES} >= 0;
subject to store_F_decom_up_to {p_decom in PHASE_UP_TO, p_built in PHASE_UP_TO union {"2015_2021"}, c in REGIONS, j in TECHNOLOGIES}:
    F_decom_up_to[p_decom,p_built,c,j] = F_decom[p_decom,p_built,c,j];

# F_used_year_start_next: Capacity at start of next window's first year
# Only indexed over YEAR_ONE_NEXT (single year), NOT accumulated
var F_used_year_start_next {YEAR_ONE_NEXT, REGIONS, TECHNOLOGIES} >= 0;
subject to store_F_used_year_start_next {y in YEAR_ONE_NEXT, c in REGIONS, j in TECHNOLOGIES}:
    F_used_year_start_next[y,c,j] = F_used_year_start[y,c,j];

# R_year_local/R_year_exterior: Annual resource consumption by source (local/exterior).
# Separated because cost and emission coefficients differ by source.
subject to compute_R_year_local {y in YEARS_WND diff YEAR_ONE, c in REGIONS, r in RESOURCES}:
    R_year_local[y,c,r] = sum {t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]}
        (R_t_local[y,c,r,h,td] * t_op[h,td]);

subject to compute_R_year_exterior {y in YEARS_WND diff YEAR_ONE, c in REGIONS, r in RESOURCES}:
    R_year_exterior[y,c,r] = sum {t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]}
        (R_t_exterior[y,c,r,h,td] * t_op[h,td]);

# Store for rolling horizon: export via fix.dat (_up_to → "" by string replacement)
var R_year_local_up_to {YEARS_UP_TO, REGIONS, RESOURCES} >= 0;
subject to store_R_year_local_up_to {y in YEARS_UP_TO, c in REGIONS, r in RESOURCES}:
    R_year_local_up_to[y,c,r] = R_year_local[y,c,r];

var R_year_exterior_up_to {YEARS_UP_TO, REGIONS, RESOURCES} >= 0;
subject to store_R_year_exterior_up_to {y in YEARS_UP_TO, c in REGIONS, r in RESOURCES}:
    R_year_exterior_up_to[y,c,r] = R_year_exterior[y,c,r];
