...
#	within its boundaries. Based on a hourly resolution, it optimizes the design and operation of the energy system while minimizing the cost of the system.
#
...
###  MILP formulation    ###
############################
set DIMENSIONS ; # := 1 .. 504;		# Number of input data per day (24h x nbr of time series)
...
### Constraints
# Allocate one cluster center (i) to each day (j)
...
# Each TD lust represent at least 1 day
subject to representation {i in DAYS}:
	sum{j in DAYS} Cluster_matrix[i,j] >= Selected_TD[i];
